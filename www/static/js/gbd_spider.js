...
function get_task_logs(){
   url = '/api/task_logs'
    $.ajax({ url: url, context: document.body, success: function(msg){

     }});
     jQuery.UIkit.modal.alert('任务执行完成...!');
}
...
function get_tushare_datas(type){
    var url = '/api/'
    if (type == 'fundamental_datas_history') {
        url += 'get_fundamental_datas_history';
    }else if(type == 'stock_classification_datas'){
        url += 'get_stock_classification_datas';
    }else if(type == 'stock_deal_data'){
         url += 'get_stock_deal_data';
    }else if(type == 'fundamental_datas_now'){
        url += 'get_fundamental_datas_now';
    }else if(type == 'stock_hist_data_multithreading'){
             url += 'get_stock_hist_data_multithreading';
    }
    ...
    $.ajax({ url: url, context: document.body, success: function(msg){
     ...
     jQuery.UIkit.modal.alert('任务启动成功...!');
}

function get_assurance_datas(){
    var url = '/api/get_assurance_data'
    $.ajax({ url: url, context: document.body, success: function(msg){
        if(msg == 'OK'){
            jQuery.UIkit.modal.alert('任务执行完成...!');
        }
...
function get_finance_news_datas(){
    var url = '/api/get_finance_news_data'
    $.ajax({ url: url, context: document.body, success: function(msg){
        if(msg == 'OK'){
            jQuery.UIkit.modal.alert('任务执行完成...!');
        }
...
$(function() {
    if (location.pathname.indexOf('/task_list')===0) {
        $('li[data-url=task_list]').addClass('uk-active');
    }else if(location.pathname.indexOf('/task_subimt')===0){
        $('li[data-url=task_subimt]').addClass('uk-active');
    }
});
