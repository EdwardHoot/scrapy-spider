...
from aperiodicity.items import GenericItem
from scrapy.linkextractors import LinkExtractor
from scrapy.spiders import CrawlSpider,Rule
class NaffmiSpider(CrawlSpider):
    name='naffmi'
    custom_settings = {
        'FIELDS_TO_EXPORT': ['id', 'source', 'url', 'title', 'date', 'body']
    }
    source = "中国银行间市场交易商协会"
    ...
    def get_title(self,response,item):
        title=response.xpath('//div[@class="Section1"]/p[10]').xpath('string(.)').extract()[0]
        if title:
            item['title']=(title+u'接受注册通知书').replace(u"：",u"").replace(u" ",u"")
