...
class CaijingSpider(CrawlSpider):
    ...
    def get_date(self,response,item):
        ...
        if date:
            ...
            datestr=''.join(''.join(date).replace(u'年', u'').replace(u'月', u'').replace(u'日', u'').replace(u':', u'').replace(u' ', u'').strip()[-12:])+'00'
            if date and datestr.isdigit():
                item['date'] = datestr
