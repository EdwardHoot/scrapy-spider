...
# env.user = 'root'
# env.sudo_user = 'root'
env.hosts = ['root@120.52.78.101:22']

db_user = 'root'
db_password = 'mysql'

_TAR_FILE = 'dist-gbd_spider.tar.gz'

_REMOTE_TMP_TAR = '/root/gbd-gface-spider/tmp/%s' % _TAR_FILE

_REMOTE_BASE_DIR = '/root/gbd-gface-spider/srv/gbd_spider'
...
def backup():
    ...
    '''
    dt = _now()
    f = 'backup-gbd_spider-%s.sql' % dt
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick gbd_spider > %s' % (db_user, db_password, f))
...
def build():
    ...
    Build dist package.
    '''
    includes = ['static', 'service','templates', 'transwarp', 'favicon.ico', '*.py']
...
def deploy():
    ...
    with settings(warn_only=True):
        run('supervisorctl stop gbd-spider')
        run('/usr/local/nginx/sbin/nginx -s reload')
        run('supervisorctl start gbd-spider')
#         ps -aux | grep "gunicorn"
#         gunicorn --bind 127.0.0.1:9000 --workers 1 --timeout 7000 --worker-class gevent wsgiapp:application
...
def rollback():
    ...
    with cd(_REMOTE_BASE_DIR):
        ...
        run('chown www-data:www-data www')
        with settings(warn_only=True):
            run('supervisorctl stop gbd_spider')
            run('supervisorctl start gbd_spider')
...
def restore2local():
    ...
    sqls = [
        'drop database if exists gbd_spider;',
        'create database gbd_spider;',
        'grant select, insert, update, delete on gbd_spider.* to \'%s\'@\'localhost\' identified by \'%s\';' % (db_user, db_password)
    ]
    for sql in sqls:
        local(r'mysql -uroot -p%s -e "%s"' % (p, sql))
    with lcd(backup_dir):
        local('tar zxvf %s' % restore_file)
    local(r'mysql -uroot -p%s gbd_spider < backup/%s' % (p, restore_file[:-7]))
    with lcd(backup_dir):
